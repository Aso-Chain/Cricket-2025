...
public class SuperChaseResult : Singleton<SuperChaseResult>
{
	...
	private IEnumerator SyncPoints()
	{
		...
		{
			yield return 0;//Server_Connection.instance.User_Leaderboard_Sync();
	...
	private void ValidateLevel()
	{
		...
		if (successfulChase)
		{
			...
			{
				//FBAppEvents.instance.LogSuperChaseEvent("SC_Win", "Win"); //Pratik
			}
		...
		else
		{
			...
			if (CONTROLLER.CTSubLevelId == 4)
			{
				//FBAppEvents.instance.LogSuperChaseEvent("SC_Loss", "Loss");  //Pratik
			}
	...
	private void LeaderBoardAnim()
	{
		int num = 1;
		num = 1;
		/*if (Singleton<Google_SignIn>.instance.googleAuthenticated == num && Application.internetReachability != NetworkReachability.NotReachable)
		{
	...
	public void ShowAvailableUpgrade()
	{
		...
		int num3 = 1;
		num3 = 1;
		if (Application.internetReachability != NetworkReachability.NotReachable /*&& Singleton<Google_SignIn>.instance.googleAuthenticated == num3*/)
		{
			for (int i = 0; i < 3; i++)
			{
